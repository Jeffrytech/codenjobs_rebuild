...
const CurrentJobPostPreview = ({ previewType, preview }) => {
  const {
    job_title,
    job_category,
...
const JobPostPreview = ({
  ...
  useEffect(() => {
    if (id !== null) {
      findJobStatusById(id).then(({ data: status, error }) => {
        // Handle error later
        if (status === "Draft") {
          setJobStatus(status);
          return;
        ...
        <JobPostPreviewContainer>
          <JobPostPreviewWrapper>
            <JobPostPreviewSection>
              {/* Extract CSS */}
              <JobPreviewHeader>
                <PreviewIntro>Preview your job post</PreviewIntro>
              ...
              <ContinueButtonWrapper>
                <a
                  ...
                  href={`/job/${
                    jobStatus === "Hold" ? "repost" : "post"
                  }/payment?&method=solana&title=${title}&id=${id}`}
                  rel="noopener noreferrer"
                >
                  <SolanaContinueButton>
                    <SolanaImage />
