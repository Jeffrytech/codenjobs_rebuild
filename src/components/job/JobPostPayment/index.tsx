...
const JobPostPayment = ({ jobId, title, user, method }) => {
  const { username } = user;
  ...
  const [
    ...
    setDisableJobPostSolanaPaymentButton,
  ] = useState(false);
  const [disableJobPostCodePaymentButton, setDisableJobPostCodePaymentButton] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (userPublicKey === null) {
      const solanaWalletButton = document.getElementsByClassName(
  ...
  useEffect(() => {
    if (connected && userPublicKey) {
      ...
      setSolanaNotification({
        ...
        message: `Your wallet is ${walletAddress.substring(
          0,
          4
  ...
  useEffect(() => {
    if (
      userPublicKey !== null &&
      jobPostPrices &&
      jobPostPrices.job_post_price_in_solana &&
      method === "solana"
    ) {
      solanaConnection.getBalance(userPublicKey).then((balance) => {
        ...
        if (currentSolanaBalance <= jobPostPrices.job_post_price_in_solana) {
          ...
          showErrorMessage(
            ...
            `The current job post price is ${jobPostPrices.job_post_price_in_solana.toFixed(
              2
            )} SOL while your balance is ${currentSolanaBalance.toFixed(
              2
            )} SOL. Please buy it more first.`
    ...
    if (
      ...
      findCodeTokenBalance(userPublicKey).then((balance) => {
        ...
        if (
          ...
          jobPostPrices.job_post_price_in_code_with_discount
        ) {
          setDisableJobPostCodePaymentButton(true);

          showErrorMessage(
            "Insufficient CODE Token",
            `The current job post price is ${jobPostPrices.job_post_price_in_code_with_discount.toFixed(
              2
            )} CODE while your balance is ${currentCodeTokenBalance.toFixed(
              2
            )} CODE. Please buy it more first.`,
  ...
  const jobPostSolanaPayment = async (e) => {
    ...
    try {
      ...
      // Extract this
      const [jobPostPDA, _] = await PublicKey.findProgramAddress(
        [
      ...
      const payJobPostTx = await program.rpc.payJobPost(username, jobId, {
        accounts: {
          user: provider.wallet.publicKey,
          jobPost: jobPostPDA,
          job: jobPublicKey,
      ...
      const payJobPostTxDone = await provider.connection.getTransaction(
        payJobPostTx
      );

      if (payJobPostTxDone === null) {
        // null doesn't mean it was failure?
      ...
      console.log(
        "newJobPost, job, jobPosted, isPaid, withSolana, jobIdCorrect, usernameCorrect"
      );
      console.log(
        newJobPost,
        job,
      ...
      if (
        ...
        if (data === true) {
          ...
          router.replace(
            `/job/post/confirmation?&title=${title}&id=${jobId}&pay_job_post_tx=${payJobPostTx}&payment_method=solana`
          );
  ...
  const jobPostCodePayment = async (e) => {
    ...
    try {
      ...
      const userCodeTokenInfo = await provider.connection.getAccountInfo(
        userCodeToken
      );
      ...
      const jobPostCodeToken = await createSplTokenAccount(
        program,
        CODE_SOLANA_TOKEN_PUBLIC_KEY
      );
      ...
      let notificationMessage = (
        <ViewOnSolscan solanaTx={payJobPostWithSplTokenTx} />
      );
      ...
      const payJobPostWithSplTokenTxDone =
        await provider.connection.getTransaction(payJobPostWithSplTokenTx);

      if (payJobPostWithSplTokenTxDone === null) {
        // null doesn't mean it was failure?
        // should done something here
      ...
      if (
        ...
        if (data === true) {
          router.replace(
            `/job/post/confirmation?&title=${title}&id=${jobId}&pay_job_post_tx=${payJobPostWithSplTokenTx}&payment_method=code`
          );
      ...
      if (
        ...
        showErrorMessage("Job post payment with CODE error", error.message);
      }
    }
  };
      ...
      <JobPostPaymentContainer>
        ...
        {userPublicKey && (
          <JobPostPaymentWrapper>
            ...
            {method === "solana" && (
              <SolanaPriceCard solanaPrice={(solanaPrice * 1).toFixed(2)} />
            )}
