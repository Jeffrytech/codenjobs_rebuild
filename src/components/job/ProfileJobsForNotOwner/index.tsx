...
const ProfileJobsForNotOwner = ({ username, sort, page }) => {
  ...
  const { setFieldValue, submitForm } = useProfileJobsForNotOwnerForm({
    username,
    sort,
    // page,
  });
  ...
  if (jobList.length === 0) {
    return (
      <ProfileList>
        <NoProfileList
          href="/jobs"
          message={"The user doesn't have any job post yet."}
        />
        <JobListSkeleton />
        <JobListSkeleton />
        <JobListSkeleton />
        ...
        (
          ...
          return (
            ...
            >
              ...
              <Link
                href={`/job?&title=${jobPageTitle(job_title)}&id=${job_id}`}
              >
