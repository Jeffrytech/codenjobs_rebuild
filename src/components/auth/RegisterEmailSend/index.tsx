...
const RegisterEmailSend = ({
  ...
  useEffect(() => {
    isUserUnconfirmed(username)
      .then(({ data: userUnconfirmed, error }) => {
        if (userUnconfirmed === false) { // The user already have passed the email verification
  ...
  return (
    ...
    >
      ...
      <div>
        <span>{username !== "" ? `${username}, ` : ""}</span> we will send you a verification email in a few minutes. Please, confirm it first before you login to your account.
      </div>
      ...
      <Button
        onClick={async (e) => {
          ...
          const confirmed = confirm(`${email} was used for sign up. Do you want to register again with another email?`);
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="register-email-resent-description">
            The account confirmation email was sent again. Please, verify your email <b>{email}</b> in a few minutes.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
