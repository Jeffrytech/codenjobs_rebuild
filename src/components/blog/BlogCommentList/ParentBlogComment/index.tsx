...
import {
  ...
  ParentBlogCommentContainer, ParentBlogCommentTextWrapper, UserCommentButtonsContainer, UserCommentMoneyButtonWrapper, 
...
function ParentBlogComment({
  ...
  const [totalMoneyVoteForBlogComment, setTotalMoneyVoteForBlogComment] = useState(blog_comment_money_voters === undefined ? 0 : blog_comment_money_voters.length);

  const totalChildBlogComments =  blog_comment_child_blog_comments === undefined ? 0 : blog_comment_child_blog_comments.length;

  // Still need this?
  // const [totalChildBlogComments, setTotalChildBlogComments] = useState(blog_comment_child_blog_comments.length);
  
  const [showParentBlogCommentEdit, setShowParentBlogCommentEdit] = useState(false);
  // const [editParentCommentText, setEditParentCommentText] = useState(blog_comment_text);
  
  const [showChildBlogCommentList, setShowChildBlogCommentList] = useState(true);
      ...
      <ParentBlogCommentListBody>
        <ParentBlogCommentListWrapper>
          ...
          <UserCommentButtonsContainer>
            ...
            {totalChildBlogComments !== 0 && <UserShowSubCommentListButtonWrapper
              onClick={() => {
