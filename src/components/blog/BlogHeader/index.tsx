...
import { OwnerButtonsContainer, OwnerDeleteButtonWrapper, OwnerEditButtonWrapper, PollButtonWrapper, UserButtonsContainer, UserCommentButtonWrapper, UserMoneyButtonWrapper } from "../../OwnerButtonsCSS";
...
import { HTTPS, COMPANY_WEBSITE, COMPANY_NAME } from "../../../config/environment";
...
const BlogHeader = ({
  ...
  const [showBlogVoteMoneyUsers, setShowBlogVoteMoneyUsers] = useState(showVoters === "true" ? true : false);
  ...
  useEffect(() => {
    findBlogVoteMoneyCount(id).then(({ data }) => {
    ...
    // if (user !== null) {
    // alert("user");
    findUserVoteMoneyForBlog(id).then(({ data }) => {
  ...
  return (
    <>
      ...
      >
        ...
        {page !== "preview" && <UserButtonsContainer>
          <UserMoneyButtonWrapper onClick={async (e) => {
            // e.preventDefault();
          ...
          >
            ...
            <img
              // src={voteMoneyForBlog ? "/static/logo_green.png" : "/static/logo.png"}
              src={"/static/logo.png"}
              style={{
                width: "1.25rem",
                height: "1.25rem",
          ...
          {totalMoneyVote !== 0 && <Tooltip title="It shows who voted for this blog post" arrow >
            <PollButtonWrapper
              onClick={() => {
      ...
      <Dialog open={showShare} onClose={handleClose} aria-labelledby="blog-post-show-social-share-buttons">
        ...
        <DialogContent>
          <section style={{
            ...
            <SocialShare
              ...
              url={`${HTTPS}${COMPANY_WEBSITE}/blog?&title=${title.replaceAll(" ", "-")}?&id=${id}`}
            />
          </section>
