...
import { portfolioTitleMaxLength, portfolioDescriptionMaxLength } from "../../../validators/maxLengths";
...
const PortfolioForm = ({
  ...
  const handleUpdatepPortfolioImage = async (e) => {
    if (e.currentTarget.files[0] && e.currentTarget.files[0].size) {
      ...
      const logoSize = e.currentTarget.files[0].size / 1024 / 1024; // 1 MB

      // alert(fileSize);
      // e.currentTarget.files[0] = null;

      if (logoSize > fileSizeLimit) {
        // @ts-ignore
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="portfolio-image-file-size-error-description">
            Your portfolio image file size is {fileSize && fileSize.toFixed(4)} MB. Please, use another file with its size less than {fileSizeLimit} MB.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
